...
class DocumentIngestionService:
    ...
    async def ingest(self, *, filename: str, content: bytes, title: str, business: str | None = None, document_type: str | None = None, region: str = "NATIONAL", authority: DocumentAuthority = DocumentAuthority.DEMO, version: str | None = None, effective_from: date | None = None, effective_to: date | None = None) -> KnowledgeDocument:
        file_hash = sha256_bytes(content)
        existing = self.repository.find_by_hash(file_hash)
        if existing and existing.status == DocumentStatus.READY:
            return existing
        validate_pdf(filename, content)
        document = KnowledgeDocument(title=title, original_filename=filename, file_hash=file_hash, business=business, document_type=document_type, region=region, authority=authority, version=version, effective_from=effective_from, effective_to=effective_to, status=DocumentStatus.UPLOADED, file_path="")
    ...
    async def reindex(self, document_id) -> KnowledgeDocument:
        ...
        return await self.ingest(filename=document.original_filename, content=content, title=document.title, business=document.business, document_type=document.document_type, region=document.region, authority=document.authority, version=document.version, effective_from=document.effective_from, effective_to=document.effective_to)
