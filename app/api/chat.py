...
async def chat(payload: ChatRequest, request: Request) -> dict[str, Any]:
    ...
    data = {"lead_results": result.get("lead_results", []), "citations": result.get("citations", []), "warnings": result.get("rag_warnings", []) + result.get("warnings", []), "criteria_snapshot_id": result.get("criteria_snapshot_id"), "research_run_id": result.get("research_run_id"), "search_plan_id": result.get("search_plan_id"), "candidate_set_id": result.get("candidate_set_id"), "resolution_run_id": result.get("resolution_run_id"), "verification_run_id": result.get("verification_run_id"), "lead_set_id": result.get("lead_set_id"), "scoring_profile_id": result.get("scoring_profile_id")}
