...
def score_leads(state: AgentState, deps: AgentDependencies) -> dict:
    ...
    lead_set, scored = deps.lead_scoring_service.score_task(task_id=task.task_id, criteria_id=state["criteria_snapshot_id"], profile_ids=state.get("verified_profile_ids", []), target_count=task.target_count or 0)
    if task:
