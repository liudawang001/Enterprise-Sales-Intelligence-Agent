...
def _lead_rows(deps: AgentDependencies, task_id: str, lead_set_id: str | None) -> list[dict]:
    ...
    if not lead_set:
        return []
    scores = [
        item for item in deps.lead_score_repository.list_for_task(task_id) if item.enterprise_id in lead_set.lead_ids
    ]
    latest = {}
    for score in scores:
...
def make_select_existing_leads(deps: AgentDependencies):
    def node(state: AgentState) -> dict:
        ...
        return {
            "lead_results": rows,
            "lead_set_id": lead_set_id,
            "verified_set_id": lead_set_id,
            "progress": {"event": "ARTIFACT_REUSED", "artifact_id": lead_set_id},
        }
...
def make_reuse_verified_profiles(deps: AgentDependencies):
    def node(state: AgentState) -> dict:
        ...
        return {
            "verified_profile_ids": profile_ids,
            "verified_count": len(profile_ids),
            "progress": {"event": "ARTIFACT_REUSED", "artifact": "verified_profiles", "count": len(profile_ids)},
        }
...
def make_filter_existing(deps: AgentDependencies):
    def node(state: AgentState) -> dict:
        ...
        for candidate in deps.research_service.repository.get_candidates(parent_id):
            if evaluator.evaluate_hard_constraints(candidate.model_dump(), criteria) != "NO_MATCH":
                ...
                deps.research_service.repository.save_candidate(
                    candidate.model_copy(update={"status": CandidateStatus.FILTERED_OUT})
                )
        filtered = deps.research_service.repository.save_candidate_set(
            CandidateSet(
                research_run_id=parent.research_run_id,
                parent_set_id=parent_id,
                stage="FILTERED_REEXECUTION",
                criteria_snapshot_id=criteria.criteria_id,
                search_plan_id=parent.search_plan_id,
        ...
        update = {
            ...
            "progress": {
                "event": "FILTER_REEXECUTION_COMPLETED",
                "reused_count": len(reused_profiles),
        ...
        if filtered.candidate_count < (task.target_count or 0):
            ...
            plan = plan.model_copy(
                update={
                    "final_scope": "DISCOVERY_REQUIRED",
                    "escalation_reason": "REUSABLE_POOL_INSUFFICIENT_AT_RUNTIME",
                }
            )
            deps.mutation_repository.save_plan(plan)
            update.update(
                {
                    "mutation_scope": "DISCOVERY_REQUIRED",
                    "reexecution_plan": plan.model_dump(mode="json"),
                    "verification_needed": False,
                }
            )
        if new_candidates:
            verification_set = deps.research_service.repository.save_candidate_set(
                CandidateSet(
                    research_run_id=parent.research_run_id,
                    parent_set_id=filtered.candidate_set_id,
                    stage="VERIFY_NEW",
                    criteria_snapshot_id=criteria.criteria_id,
                    search_plan_id=parent.search_plan_id,
            ...
            update.update(
                {
                    "candidate_set_id": verification_set.candidate_set_id,
                    "researched_candidate_set_id": verification_set.candidate_set_id,
                }
            )
        else:
...
def make_targeted_enrichment(deps: AgentDependencies):
    async def async_node(state: AgentState) -> dict:
        ...
        for candidate in deps.research_service.repository.get_candidates(set_id):
            await deps.research_service.targeted_enrich_fields(
                candidate_set.research_run_id, candidate.candidate_id, fields
            )
        return {
            "candidate_set_id": set_id,
            "researched_candidate_set_id": set_id,
            "verification_needed": True,
            "reused_verified_profile_ids": [],
            "progress": {"event": "TARGETED_ENRICHMENT_COMPLETED", "fields": fields},
        }
...
def merge_verified_profiles(state: AgentState) -> dict:
    reused = (
        state.get("reused_verified_profile_ids", [])
        if state.get("mutation_scope") in {"FILTER_ONLY", "ENRICHMENT_REQUIRED"}
        else []
    )
    ...
    def promote(state: AgentState, fence_allowed: bool = True) -> dict:
        ...
        if state.get("mutation_scope") == "NONE":
            ...
            if plan_id:
                ...
                if plan:
                    deps.mutation_repository.save_plan(
                        plan.model_copy(update={"status": ReexecutionPlanStatus.COMPLETED})
                    )
            return {
                "execution_snapshot_id": current.snapshot_id if current else None,
                "progress": {"event": "ARTIFACT_REUSED"},
            }
        ...
        criteria_snapshot_id = (
            reuse.get("criteria_snapshot_id") if scope == "DISPLAY_ONLY" else state.get("criteria_snapshot_id")
        )
        ...
        filtered_candidate_set_id = (
            state.get("filtered_candidate_set_id")
            if scope == "FILTER_ONLY"
            else reuse.get("filtered_candidate_set_id")
            if partial
            else state.get("filtered_candidate_set_id")
        )
        researched_candidate_set_id = (
            reuse.get("researched_candidate_set_id")
            if partial
            else state.get("researched_candidate_set_id") or state.get("candidate_set_id")
        )
        ...
        if not fence_allowed:
            deps.execution_snapshot_repository.save(
                snapshot.model_copy(update={"is_current": False, "validity": "SUPERSEDED"})
            )
            metrics.add("stale_promotion_rejected_total")
            return {
                "execution_snapshot_id": snapshot.snapshot_id,
                "progress": {"event": "ARTIFACT_PROMOTION_REJECTED"},
                "warnings": ["STALE_RUN_FENCE"],
            }
        ...
        if plan_id:
            ...
            if plan:
                deps.mutation_repository.save_plan(
                    plan.model_copy(
                        update={
                            "status": ReexecutionPlanStatus.COMPLETED if promoted else ReexecutionPlanStatus.SUPERSEDED
                        }
                    )
                )
        return {
            ...
            "warnings": [] if promoted else ["STALE_EXECUTION"],
        }
